...
A production-grade ecommerce experience for **Appu Kaju** (Lucknow, since 1998), in an earthy editorial style — parchment, forest green and terracotta with big Zodiak serif headlines. The homepage opens on a **static full-screen cinematic hero** (Appu the elephant at golden dawn, serif tagline *Nature's Finest. Delivered Fresh.*, load-time entrances only — no pinned scroll choreography, removed by request) and flows into magazine-style sections, a sticky product showcase, and the catalogue.
...
components/
  home/                 # story chapters (hero, legacy, sourcing map, craft, …)
    hero-section.tsx    #   static full-screen cinematic hero (load-time
                        #   entrances only; no scroll choreography)
...
### The hero, briefly
A single full-screen photograph (`/images/story/appu.webp`, generated by `scripts/fetch-story.mjs`) with masked-text entrance animations that play once on load. There is deliberately **no pinned scrollytelling** — earlier iterations (packet-pour, table film, Appu Story film) live in git history if ever wanted back. `--header-fg` is parchment while the dark hero is at the top of the page; `ThemeZone` manages it from the first scroll onward. Reduced-motion users see the same layout without entrances (global CSS gate).
